...
当前 Docker 镜像对外暴露 `3000`，由 Next.js 提供页面入口，并把 `/api/*` 代理到容器内部 Go API。运行入口是 `node /app/scripts/docker-start.mjs`：先迁移数据库，再同时启动 Go API 和 Next.js；任一子进程退出时，脚本会终止另一个进程并让容器退出，避免半活状态。
...
- SQLite 默认 DSN 是 `data/infinite-canvas.db`，Docker 场景需要确保 `data` 目录有持久化挂载。
- MySQL / PostgreSQL 通过 `STORAGE_DRIVER` 和 `DATABASE_DSN` 配置。
