...
+ [待测修复] 收敛画布媒体恢复、生成运行态并发清理、视频生成错误展示、视频 JSON blob 错误拒绝和轮询超时边界。
+ [待测修复] 收敛后端 AI 代理状态码、后台渠道错误识别、扣费退款、视频任务绑定渠道、幂等退款和事务一致性边界。
+ [待测重构] 后端路由注册、显式数据库迁移和 Docker 运行入口按当前部署边界整理。
+ [测试] 增加前端 Vitest、后端 Go 回归测试和 Docker supervisor Node 测试，覆盖本轮关键修复。
+ [回归] 不依赖外部 AI Key 的画布 zip 导入导出、zip 包内媒体恢复、图片撤销恢复、“我的素材” zip 导入导出和未登录顶部导航行为已从待测移出，并保留回归手册。
+ [文档] 增加项目冷启动、当前状态、agent 分工、初始问题审查和待测验收入口。
...
+ [文档] 增加 Render 部署说明入口，实际可用性以当前部署文档和待测状态为准。
